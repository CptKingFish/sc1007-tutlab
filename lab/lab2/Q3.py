...
def palindrome(word):
    cleaned_string = "".join(c.lower() for c in word if c.isalnum())
    stack = Stack()

    for i in range(len(cleaned_string) // 2):
        stack.push(cleaned_string[i])

    start = (len(cleaned_string) + 1) // 2

    for j in range(start, len(cleaned_string)):
        if stack.is_empty() or stack.pop() != cleaned_string[j]:
